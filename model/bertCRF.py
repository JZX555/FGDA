...
class ConditionalRandomField(torch.nn.Module):
    ...
    #     logits = logits.transpose(0, 1).contiguous()
    #     mask = mask.transpose(0, 1).contiguous()
    #     log_tags = tags.transpose(0, 1).contiguous().softmax(-1).add(1e-9).log()  # 归一概率

    #     # Initial alpha is the (batch_size, num_tags) tensor of likelihoods combining the
    #     # transitions to the initial states and the logits for the first timestep.
    #     alpha = logits[0] + log_tags[0]
    #     if self.include_start_end_transitions:
    #         alpha += self.start_transitions.view(1, num_tags)
    ...
    #         buffer = logsumexp(inner, 1) + log_tags[i]  # 加上概率

    #         # In valid positions (mask == True) we want to take the logsumexp over the current_tag dimension
    #         # of `inner`. Otherwise (mask == False) we want to retain the previous alpha.
    #         alpha = buffer * mask[i].view(batch_size, 1) + alpha * (
    #             ~mask[i]
    #         ).view(batch_size, 1)
    ...
    def forward(
        ...
        if tags.dim() > 2:
            log_numerator = self._joint_likelihood_prob(inputs, tags, mask)
        else:
...
class CRF(nn.Module):
    ...
    def compute_kl_loss(self, mu: torch.FloatTensor, var: torch.FloatTensor):
