...
class AdapterBertModel(nn.Module):
    ...
    def _load_param(self, path, device):
        ...
        self.adapters[0].load_state_dict(state_dict[f'adapter_0'])
        self.adaptersr[1].load_state_dict(state_dict[f'adapter_1'])
        print(f'load pos params from {path} into device cuda: {device}')

    def _add_cur_pos_linear(self):
        self.adapters[0].add_new_parameters()
        self.adapters[1].add_new_parameters()
...
class ShareModel(nn.Module):
    def __init__(self, bert_path, label_vocab, d_model: int = 768, adapter_size: int = 192, trainsets_len=None, combine: bool = False):
        ...
        self.trainsets_len = 1
        self.lstm_dropout = nn.Dropout(p=0.2)

        self.bert = AdapterBertModel(name_or_path_or_model=bert_path, adapter_size=adapter_size, trainsets_len=self.trainsets_len, combine=combine)
